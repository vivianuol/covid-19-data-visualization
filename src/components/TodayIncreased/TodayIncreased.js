...
const TodayIncreased = ( props ) => {
    ...
    useEffect(() => {
        ...
        d3.select(circularRef.current).selectAll('svg').remove();

        // append the svg object
        var svg = d3.select(circularRef.current)
        .append("svg")
        ...
        .domain([0, 150000]); // Domain of Y is from 0 to the max seen in the data
