...
import population from "../../data/us-states-population-est2019-01.json";
...
const MapChart = () => {
        ...
        json("https://api.covidtracking.com/v1/states/current.json")
            .then(states => {
                ...
                const formattedData = states.map(state => {
                    ...
                    return ({
                        id: stateId,
                        positiveRate: statePopulation ? state.positive / statePopulation : 0
                    })
                })
    ...
    const colorScale = scaleQuantile()
        .domain(data.map(d => d.positiveRate))
        .range([
            "#ffedea",
            "#ffcec5",
            "#ffad9f",
                                ...
                                {geographies.map(geo => {
                                    ...
                                    return (
                                        <Geography
                                            key={geo.rsmKey}
                                            geography={geo}
                                            fill={cur ? colorScale(cur.positiveRate) : "#EEE"}
                                        />
                                    );
                                })}
                                {geographies.map(geo => {
